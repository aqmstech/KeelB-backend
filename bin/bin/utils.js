...
const UtilsService = {
    ...
    generateModule: async (model_name, ext, generator = null) => {
        const schemaFileExist = await UtilsService.isSchemaExist(model_name.toLowerCase())
        if (!schemaFileExist) {
        ...
        await UtilsService.generateInterface(model_name, ext);
        await UtilsService.generateService(model_name, ext);
        await UtilsService.generateModel(model_name, ext);
        await UtilsService.generateController(model_name, ext);
        await UtilsService.generateControllerAdmin(model_name, ext);
        await UtilsService.generateRoute(model_name, ext);
        await UtilsService.generateAdminRoute(model_name, ext);
        await UtilsService.generatePostmanCollection(model_name, ext);
        //await UtilsService.generateTestCase(model_name, ext);
        console.log("-------------" + model_name + " Module Generated successfully-----------")
    },
    ...
    generateInterface: async (model_name, ext) => {
        console.log("Creating Interface...", model_name)
        let dir = `./module/${model_name}`
    ...
    generateRoute: async (model_name, ext) => {
        ...
        const indexFileData = await UtilsService.replaceIndexFileData({
            route_name,
            routerName,
    ...
    generatePostmanCollection: async (model_name, ext) => {
        try {
            ...
            // Create a new file for the generated model
            const newFile = `${collectionDir}/${model_name.toLowerCase()}.postman_collection`;

            fs.writeFileSync(newFile, JSON.stringify(newRequest, null, 2), 'utf8');
            console.log(`New model file created: ${newFile}`);
        } catch (error) {
    ...
    generateValidator: async (model_name, ext) => {
        ...
        let file_name = `${model_name.toLowerCase()}Validator.${ext}`;
    ...
    generateTestCase: async (model_name, ext_name) => {
        try {
            ...
            const file_path = `./test/${file_name}`;

            // Check if the file already exists
            if (!fs.existsSync(file_path)) {
                // File doesn't exist, generate and write the test case data
                const testCaseData = await UtilsService.replacetestCaseData(model_name, ext_name);
                console.log('testCaseData-->', testCaseData);
                fs.writeFileSync(file_path, testCaseData);
                console.log('Test Case Created Successfully');
            } else {
                // File already exists, do nothing
                console.log('Test Case File Already Exists');
            }

            return true;
        } catch (error) {
            console.error('Error occurred while generating test case:', error);
            return false;
        }
    },
    ...
    removeModule: async (model_name, ext) => {
        ...
        UtilsService.removeFileIfExists(`bin/postman-collection/${model_name}.postman_collection`);
    ...
    generateJoiSchema: async (model, data) => {
        ...
        schemaColumn.get['filter'] = `Joi.any()`
