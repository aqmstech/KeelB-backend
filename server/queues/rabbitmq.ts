...
export abstract class RMQ {
    ...
    return {
      vhosts: {
        '/': {
          ...
          queues: {
            /** Following Section Will have Queue Configuration if any. Note Keyname doesn't have in impact but it is useful when subcribing to them then keyname will be used to acquire the configurations */
            smtp: {
              ...
              options: {
                ...
                deadLetterExchange: 'deadletter' /** In case of too many failure or any other case for which we don't want to receive event any further that event will be published to this Exchange for manual use or any other action/monitoring etc  */,
          ...
          exchanges: {
            /** Exchanges are front-end router of queues. When Queue is bounded behind exchange the routing strategies can be configured on Exchange for appropriate queue routing */
            deadletter: {
          ...
          bindings: {
            /** Binding works as glue between Exchange and queue. Rules between exchange and queues will be configured here  */
            smtp: {
              source: 'notification_exchange' /** Name of Exchange for which we want to create binding */,
              destination: 'smtp' /** Name of destination where exchange will deliver message */,
              destinationType: 'queue' /** Destination can be queue and any other exchange as well. In our case we will using queue */,
              bindingKey: 'smtp' /** routing key. Currently we are using simple strings for routing but expressive and pattern based routing can also be done. See more details at RabbitMQ official guide */,
            },
            fcm: {
          ...
          publications: {
            ...
            smtp: {
              exchange: 'notification_exchange' /** Destination where this type of event will be published for consumption. @NOTE We're submitting to exchange if you want to submit directly to queue uncomment following property and comment this  */,
              ...
              routingKey: 'smtp' /** Useful when using exchanged it will route the data to appropriate queue for consumption */,
            }
          }
        },
      },
  ...
  public static async CHECKHEALTH() {
    try {
      let temp = await BrokerAsPromised.create(this.conf);
      temp.shutdown();
      return true;
    } catch (error) {
      throw error;
  ...
  public static async INIT(conf: RabbitMQ) {
    try {
      ...
      // await this.SubscribeChannel('test');
    } catch (error) {
      console.log('Error in INIT QUEUE');
      console.log(error);
      this.broker = undefined as any;
      throw error;
    }
  }
  ...
  public static async PublishMessageConfirm(queueName: 'fcm' | 'smtp' , payload: { msg: string; data: any }): Promise<string> {
    return new Promise(async (resolve, reject) => {
      try {
        let session: PublicationSession;
        let validQueueName = false;
        let timeout = setTimeout(() => {
          throw new Error(`Publishing Timeout Occurred : ${queueName} : Payload : ${JSON.stringify(payload, undefined, 4)}`);
        }, this.publishTimeout);
   ...
   * @returns string
   */
  public static async PublishMessage(queueName: 'fcm' | 'smtp', payload: { msg: string; data: any }): Promise<void> {
    return new Promise(async (resolve, reject) => {
      try {
        let session: PublicationSession;
        let validQueueName = false;
  ...
  public static async SubscribeChannel(queueName: string) {
    try {
      ...
      session.on('invalid_content', (message, content, ackorNak) => {
        ...
        ackorNak(new Error(JSON.stringify(content)), [
          {
            immediateNack: true,
            requeue: false,
            xDeathFix: true,
