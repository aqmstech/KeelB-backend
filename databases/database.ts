    ...
    public static async Connect(dbconf: DBConfigMongo) {
        try {
            ...
            if (!this.mongClient || !this.db) {
                if (dbconf.atlas) this.mongClient = await MongoClient.connect(dbconf?.auth ? `mongodb+srv://${dbconf.auth?.username}:${dbconf.auth?.password}@${dbconf.host}` : `mongodb+srv://${dbconf.host}`, {
                    retryWrites: true,
                    w: 'majority'
                });
                else this.mongClient = await MongoClient.connect(dbconf?.auth ? `mongodb://${dbconf.auth?.username}:${encodeURIComponent(dbconf.auth?.password as string)}@${dbconf.host}:${dbconf.port}` : `mongodb://${dbconf.host}:${dbconf.port}`, {
                    retryWrites: true,
                    w: 'majority'
                });
